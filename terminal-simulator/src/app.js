...
${indent}</${name}>`;
  ...
  function buildDatabasePreview(card, decision) {
    ...
    const requestLines = [
      ...
      "  RETURNING id",
      "), basket AS (",
      "  INSERT INTO basket_data (total_amount, pre_auth_amount, currency, request_info_id)",
      ...
      "  RETURNING id",
      "), item AS (",
      "  INSERT INTO sale_items (basket_data_id, product_code, amount, quantity, add_prod_code, reverse_sale, sale_channel, rebate_label, add_prod_info)",
      "  SELECT " + ["basket.id", sqlValue(t.productCode), sqlValue(toMoneyString(t.amount)), sqlValue(t.quantity), "NULL", sqlValue(t.type === "Refund" ? "true" : "false"), "'EPT_SIM'", "NULL", sqlValue(t.productName + "; unit=" + t.unitPrice + "/" + t.unitMeasure + "; tax=" + t.taxCode)].join(", "),
      "  FROM basket",
      "  ON CONFLICT (basket_data_id, product_code) DO UPDATE SET amount = EXCLUDED.amount, quantity = EXCLUDED.quantity",
      "  RETURNING id",
      ")"
    ];
    ...
    lines.push(lblTotal + " ".repeat(40 - lblTotal.length - amountStrFormatted.length) + amountStrFormatted);
      ...
      if (transaction.split && sessionToUse && sessionToUse.payments) {
        sessionToUse.payments.forEach(payment => {
          ...
          lines.push(lblCard + " ".repeat(40 - lblCard.length - maskedCard.length) + maskedCard);
        ...
        lines.push(divider);
        const maskedCard = "**** " + card.number.slice(-4);
        lines.push(lblCard + " ".repeat(40 - lblCard.length - maskedCard.length) + maskedCard);
